...
/*
    TelephonyFeature.CHINA_CTC_DATA [
    Add this file
    ]
*/
...
#define NUM_PDP_CONTEXT			7		/* number of PDP context */
#else
...
#if DGS_TEST
	typedef struct{
		UINT8 header_code[4];  // magic ???   // {0x7F,0xAA,0xAF,0x7E}
		UINT8 model[20];       // ????
		UINT8 nature[40];      // ??????
		UINT8 custt_code[8];   // ?«ô???
		UINT8 date[14];        // ???? ??????
		UINT8 charger[24];     // ??????		UINT8 version[20];     // SW ??????
...
void request_phone_power_off_reset(int flag)
{
	...
	switch(flag) {
		...
		case POWEROFF : 
			...
			disable_irq(INT_A9_M2A_3);
			printk("Dpram interrupt disabled\n");
			break;
	}
}
		                    ...
		                    unsigned long count, void *data)
{
	...
	printk("smem_flag : 0x%x\n", (unsigned int)smem_flag);
...
static int __init dpram_init(void)
{
	...
	#if defined(CONFIG_MACH_TASS_CTC)|| defined(CONFIG_MACH_COOPER_CTC) // final feature 
	smem_flag->info = 0xAEAEAEAE;
	#endif
...
#ifdef CONFIG_SAMSUNG_CTC_CW		
		case 36:	start = 5;	break;
		case 41:	start = 6;	break;
#endif		
		default:	       start = 0;
	}
