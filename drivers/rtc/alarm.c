...
#define ANDROID_ALARM_PRINT_DEBUG (1U << 7)

static int debug_mask = ANDROID_ALARM_PRINT_ERROR | \
			ANDROID_ALARM_PRINT_INIT_STATUS | \
			ANDROID_ALARM_PRINT_SUSPEND | \
			ANDROID_ALARM_PRINT_DEBUG;
...
int alarm_set_rtc(struct timespec new_time)
{
	...
	spin_unlock_irqrestore(&alarm_slock, flags);
	pr_alarm(DEBUG, "%s: elapsed updated = %d\n", __func__, alarms[ANDROID_ALARM_ELAPSED_REALTIME].delta); 
...
ktime_t alarm_get_elapsed_realtime(void)
{
	...
	spin_unlock_irqrestore(&alarm_slock, flags);
	//pr_alarm(DEBUG, "stop=%d, %d - %d = %d\n", base->stopped, now2.tv.sec, base->delta.tv.sec, now.tv.sec); 
	return now;
}

int alarm_is_suspend(void)
{
	return suspended; 
}
