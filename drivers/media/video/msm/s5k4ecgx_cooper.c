...
//kk0704.park :: Protect entering sleep while Camera preview starting
struct wake_lock s5k4ecgx_wake_lock;
...
#define CAM_FLASH_FLEN 3 //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_1 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_1);break; //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_2 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_2);break; //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_3 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_3);break; //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_4 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_4);break; //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_5 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_5);break; //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_6 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_6);break; //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_7 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_7);break; //CTC_MAIN
		...
		case EXT_CFG_ZOOM_STEP_8 :
			if(s5k4ecgx_status.camera_mode == EXT_CFG_CAM_MODE_CAMERA)
			{
				switch(s5k4ecgx_status.snapshot_size)
				{
					...
					case EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA: S5K4ECGX_WRITE_LIST(s5k4ecgx_X2_Zoom_8);break; //CTC_MAIN
...
void s5k4ecgx_set_capture_size(int size)
{
	...
	else if(size == EXT_CFG_SNAPSHOT_SIZE_1024x768_XGA)S5K4ECGX_WRITE_LIST(s5k4ecgx_XGA_Capture) //CTC_MAIN
...
void s5k4ecgx_set_preview(void)
{
	...
	//kk0704.park :: Protect entering sleep while Camera preview starting
	wake_lock_timeout(&s5k4ecgx_wake_lock, 5 * HZ );
	printk("%s :: Protect entering Sleep Mode while PREVIEW STARTING~! \n", __func__);
...
void s5k4ecgx_set_power(int status)
{
	...
	if(status == 1) //POWER ON
	{
		...
		gpio_tlmm_config(GPIO_CFG(2, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), GPIO_CFG_ENABLE);

		gpio_set_value(2,1);
		msleep(1);

		gpio_tlmm_config(GPIO_CFG(31, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), GPIO_CFG_ENABLE);

		gpio_set_value(31,1);
		msleep(1);

		gpio_tlmm_config(GPIO_CFG(27, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), GPIO_CFG_ENABLE);

		gpio_set_value(27,1);
		msleep(1);

		vreg_enable(vreg_cam_out9);//AF
#ifdef WORKAROUND_FOR_LOW_SPEED_I2C
...
//hmin84.park 101130
	...
	//kk0704.park :: Protect entering sleep while Camera preview starting	
	wake_lock_init(&s5k4ecgx_wake_lock, WAKE_LOCK_SUSPEND, "camsensorsleep");
	printk("%s :: added s5k5ccff_wake_lock \n", __func__);
