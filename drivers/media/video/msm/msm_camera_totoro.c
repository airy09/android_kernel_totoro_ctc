	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_GET_CAMERA_INFO:
		...
		break;
#if defined(CONFIG_MACH_TASS) || defined(CONFIG_MACH_BENI) || defined (CONFIG_MACH_GIO) || defined(CONFIG_MACH_LUCAS) || defined(CONFIG_MACH_CALLISTO)
...
static int __msm_release(struct msm_sync *sync)
{
	...
	if (!sync->opencnt) {
		...
		if (sync->core_powered_on) {
			...
			//kk0704.park changed power off sequence from "sensor off -> clk off" to "clk off->sensor off"
