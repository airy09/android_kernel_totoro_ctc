...
struct wake_lock s5k5ccff_wake_lock;
...
#define S5K5CCFF_BURST_WRITE_LIST(A)	s5k5ccff_sensor_burst_write_list(A,ARRAY_SIZE(A),#A);
#define S5K5CCFF_WRITE_LIST(A)			s5k5ccff_sensor_write_list(A,ARRAY_SIZE(A),#A);
	...
	switch(ctrl_info.mode)
	{
		...
		case PCAM_AWB_AE_CONTROL:
		{
			...
			switch(ctrl_info.value_1)
			{
				...
				S5K5CCFF_WRITE_LIST(s5k5ccff_awb_ae_lock);
				}
				break;
				...
				S5K5CCFF_WRITE_LIST(s5k5ccff_awb_ae_unlock);
				}
				break;
...
void cam_pw(int status)
{
	...
	{
	//kk0704.park for TASS CTC Camera 

		gpio_tlmm_config(GPIO_CFG(2,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_set_value(2, 1); //CAM_EN :: VDDA, VDD_REG
		mdelay(1);
		gpio_tlmm_config(GPIO_CFG(3,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		gpio_set_value(3,1); // VDDIO
		udelay(100);
		/* Input MCLK = 24MHz */
	}
	else //POWER OFF
	{
		gpio_set_value(2, 0); //CAM_EN :: VDDA, VDD_REG
		gpio_set_value(3, 0); // VDDIO
	}
}
...
//	unsigned int	before_time, after_time, i;//I2C SPEED TEST
	...
	udelay(100);
	gpio_tlmm_config(GPIO_CFG(1,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_set_value(1, 1); //CAM_STBY
	udelay(100);
	gpio_tlmm_config(GPIO_CFG(0,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	...
	if(id != 0x05CC)
	{
		...
		rc = -1;		//kk0704.park :: No sensor ERROR 
	}
	else
...
static long s5k5ccff_set_sensor_mode(int mode)
{
	...
	i2c_clk_addr = get_i2c_clock_addr(s5k5ccff_client->adapter);
	...
	if(first_start_camera)
	{
		...
		//kk0704.park :: increase I2C Speed
		...
		pcam_msm_i2c_pwr_mgmt(s5k5ccff_client->adapter, 0);
		msleep(3);
		//kk0704.park :: increase I2C Speed*/
	...
	case SENSOR_PREVIEW_MODE:
	{
		...
		//kk0704.park :: Protect entering sleep while Camera preview starting
		wake_lock_timeout(&s5k5ccff_wake_lock, 5 * HZ );
...
{
	//kk0704.park :: Protect entering sleep while Camera preview starting	
	wake_lock_init(&s5k5ccff_wake_lock, WAKE_LOCK_SUSPEND, "camsensorsleep");
