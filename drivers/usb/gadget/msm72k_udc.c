...
#define	DRIVER_NAME_USBS		"MSM72K_UDC_USBS"
...
static void usb_do_check_state(struct work_struct *w);
...
struct usb_info {
	...
	*/
	...
	struct work_struct work_usb_state;
	unsigned phy_status;
...
{
	return (int)the_usb_info->usb_state;
}
EXPORT_SYMBOL(get_usb_status);
...
static ssize_t print_switch_name_usbs(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "%s\n", DRIVER_NAME_USBS);
}

static ssize_t print_switch_state_usbs(struct switch_dev *sdev, char *buf)
{
	struct usb_info *ui = the_usb_info;
...
static void handle_setup(struct usb_info *ui)
{
	...
	if (ctl.bRequestType == (USB_DIR_OUT | USB_TYPE_STANDARD)) {
		if (ctl.bRequest == USB_REQ_SET_CONFIGURATION) {
			...
			schedule_work(&ui->work_usb_state);
...
static void usb_prepare(struct usb_info *ui)
{
	...
	INIT_WORK(&ui->work_usb_state, usb_do_check_state);
...
static void usb_do_check_state(struct work_struct *w)
{
	switch_set_state(&the_usb_info->sdev_usbs, the_usb_info->usb_state);
...
static int msm72k_probe(struct platform_device *pdev)
{
	...
	ui->sdev_usbs.name = DRIVER_NAME_USBS;
	ui->sdev_usbs.print_name = print_switch_name_usbs;
	ui->sdev_usbs.print_state = print_switch_state_usbs;

	retval = switch_dev_register(&ui->sdev_usbs);
	if (retval)
		return usb_free(ui, retval);
	...
	if (retval) {
		...
		switch_dev_unregister(&ui->sdev_usbs);
