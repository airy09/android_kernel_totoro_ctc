...
#define SEC_OTG_CHECK_INTERVAL	( 3 * HZ ) // every 1 sec

void sec_otg_registertimer(void); 
static void sec_otg_check_event(struct work_struct *work);
static void sec_otg_deregistertimer(void);

static DECLARE_WORK(sec_otg_work, sec_otg_check_event);

struct workqueue_struct *  sec_otg_wq;
struct timer_list sec_otg_timer;
static int sec_otg_timer_enable= 0;
static struct wake_lock sec_otg_wake_lock;
...
static int msm_otg_runtime_idle(struct device *dev)
{
 ...
 int msm_otg_force_suspend(void)
{
	struct msm_otg *dev = the_msm_otg;
	
	return msm_otg_suspend( dev ); 
}
EXPORT_SYMBOL( msm_otg_force_suspend ); 


#ifdef CONFIG_OTG_RELEASE_BUG_FIX 
static void sec_otg_timeover(unsigned long arg )
{
	printk("[%s]\n", __func__); 

	queue_work(sec_otg_wq, &sec_otg_work);
}

void sec_otg_registertimer(void)
{
	struct timer_list* ptimer = &sec_otg_timer; 

	printk("[%s]\n", __func__); 

	if(sec_otg_timer_enable) {
		del_timer( ptimer );
	} 
	else {
		wake_lock(&sec_otg_wake_lock);
	}

	init_timer( ptimer );
	ptimer->expires = get_jiffies_64() + SEC_OTG_CHECK_INTERVAL;
	ptimer->data = (long) NULL;
	ptimer->function = sec_otg_timeover;
	add_timer( ptimer );
	sec_otg_timer_enable =1;

}
EXPORT_SYMBOL(sec_otg_registertimer); 
...
static void sec_otg_deregistertimer(void)
{
	...
	if(sec_otg_timer_enable){
		del_timer( ptimer );
		sec_otg_timer_enable =0;
	}
	
	wake_unlock(&sec_otg_wake_lock);	
}

static void sec_otg_check_event(struct work_struct *work)
{
	struct msm_otg *dev = the_msm_otg;
	int lpm = atomic_read(&dev->in_lpm); 
	
	printk("[%s]  cable=%d, LPM=%d\n", __func__, tsp_charger_type_status, lpm); 

	//msm_otg_irq( 0, (void *)the_msm_otg ); 
	if ( !tsp_charger_type_status && !lpm) { /* no cable OTG running condition by noise */
		msm_otg_force_suspend(); 
		wake_unlock(&dev->wlock);
	}
...
{
#ifdef CONFIG_OTG_RELEASE_BUG_FIX 
	wake_lock_init(&sec_otg_wake_lock, WAKE_LOCK_SUSPEND, "sec_otg_lock");

	sec_otg_wq = create_singlethread_workqueue("sec_otg");
	if (!sec_otg_wq) {
