...
#define Digi_HEX_SW_VER 0x0D

#define SMAC_HEX_HW_VER 0x11
#define SMAC_HEX_SW_VER	0x08	//change the version while Firmware Update 
...
int set_tsp_for_cam_detect(int);
...
static void check_ic_work_func(struct work_struct *work)
{
	...
	uint8_t wdog_val[1];
	uint8_t i2c_addr_pam = 0x02;
	uint8_t pam_val[1];
	uint8_t pam_f_value=0;
	uint8_t pam_s_value=0;

		if( pre_cam_stat != tsp_cam_irq_status )
		{
			set_tsp_for_cam_detect(tsp_cam_irq_status);
		}	
	...
	if (ret <= 0) {
		...
		i2c_error_cnt = 0;
		ret = tsp_i2c_read( i2c_addr_pam, pam_val, sizeof(pam_val));
		pam_f_value = pam_val[0] & 0x30;
		if(pam_f_value){
			pam_s_value = pam_val[0] & 0x07;
			printk("[TSP] %s -pamtouch : fr=%d, se=%d\n", __func__,pam_f_value,pam_s_value);
		}
	...
	{
		printk("[TSP]  wdog reset=%d(%d) \n",wdog_val[0],prev_wdog_val, i2c_error_cnt);
		disable_irq(ts_global->client->irq);
		tsp_reset();
		enable_irq(ts_global->client->irq);
		i2c_error_cnt=0;
	...
	printk("[TSP] %s, ic_cnt=%d\n", __func__, i2c_error_cnt );
...
int set_tsp_for_cam_detect(int state)
{
	...
	ret = tsp_i2c_read( i2c_addr, wdog_val, sizeof(wdog_val));                 
	printk("[TSP] %s = %x, set=%d\n", __func__, (wdog_val[0] & 0x20),state );
}
EXPORT_SYMBOL(set_tsp_for_cam_detect);
...
int set_tsp_for_ta_detect(int state)
{
	...
	ret = tsp_i2c_read( i2c_addr, wdog_val, sizeof(wdog_val));                 
	printk("[TSP] %s = %x, set=%d\n", __func__, (wdog_val[0] & 0x10),state );
}
