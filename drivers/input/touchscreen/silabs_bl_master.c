...
#include "silabs_FIRMWARE/SILABS_S5360_SMAC005_BL006_APP020.h"
#include "silabs_FIRMWARE/SILABS_S5360_SMAC007_BL006_APP020.h"
...
int WriteBytes (unsigned int addr, unsigned int num)
{
   ...
   if(TSP_MODULE_ID==0x05)
   {
	    for(i=0;i<num;i++) SMB_DATA_OUT[i+9] = SMAC005_APP020_Binary[addr+i];
   }
   else if(TSP_MODULE_ID==0x07)
   {
	    for(i=0;i<num;i++) SMB_DATA_OUT[i+9] = SMAC007_APP020_Binary[addr+i];
   }
...
int GetPageCRC (unsigned int addr)
{
   ...
   {    
		if(TSP_MODULE_ID==0x05)
			write_data = SMAC005_APP020_Binary[addr+i];
		if(TSP_MODULE_ID==0x07)
			write_data = SMAC007_APP020_Binary[addr+i];
