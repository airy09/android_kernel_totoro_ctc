...
static irqreturn_t melfas_ts_irq_handler(int irq, void *dev_id)
{
	struct melfas_ts_data *ts = dev_id;
       ...
       #ifdef __TOUCH_DEBUG__
	printk("[TSP] %s: TSP_INT=%d, %d\n", __func__,  gpio_get_value(TSP_INT), __LINE__);
       #endif
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[TSP] firmware version = 0x%x", buf_firmware);
    ...
    if (client->irq) {
		ret = request_irq(client->irq, melfas_ts_irq_handler, IRQF_TRIGGER_LOW |IRQF_DISABLED, client->name, ts);
