...
#if defined(CONFIG_MACH_TASS_CTC) || defined(CONFIG_MACH_COOPER_CTC)
#define GPIO_KBR0 42
#define GPIO_KBR1 41
#define GPIO_KBR2 40
#define GPIO_KBR3 39
...
#define SLEEP_STATE_SKIP_LONGKEY
#endif
...
#define MAX_KEY_PRESS_COUNT   80 //300
...
unsigned short current_pressed_key = 0;
#endif
...
#if defined(CONFIG_MACH_TOTORO_CTC)
#define KEY_HW_REV(a)		mi->keymap[a]
#elif defined(CONFIG_MACH_TASS_CTC)
static unsigned short key_chg_map[] = {
 	[0] = KEY_MENU,
	[1] = KEY_VOLUMEUP,
	[2] = KEY_BACK,
	[3] = KEY_RESERVED,
...
#define KEY_HW_REV(a)		( board_hw_revision <6 ) ? key_chg_map[a] : mi->keymap[a]
#else
#define KEY_HW_REV(a)		mi->keymap[a]
#endif
...
static irqreturn_t gpio_keypad_irq_handler(int irq_in, void *dev_id)
{
	...
	if(long_key_state == LONG_KEY_PRESSED)
	{
	        if (current_pressed_key == KEY_VOLUMEDOWN)
		{
		        printk("******** volume down key release ********\n");
			irq = gpio_to_irq(42);				
			set_irq_type(irq , IRQ_TYPE_LEVEL_LOW);
		}
		if(current_pressed_key == KEY_VOLUMEUP)
		{
		        printk("******** volume up key release ********\n");
			irq = gpio_to_irq(41);				
			set_irq_type(irq , IRQ_TYPE_LEVEL_LOW);
		}
		if(current_pressed_key == KEY_END)
		{
