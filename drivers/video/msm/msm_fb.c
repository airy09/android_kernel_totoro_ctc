...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	/* Mark for GetLog */
	#if defined(CONFIG_MACH_COOPER_CTC) ||  defined(CONFIG_MACH_TASS_CTC) 
	frame_buf_mark.p_fb = fbram_phys - 0x13000000;
	#else
...
static int msm_fb_suspend_sub(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->op_enable) {
		...
		if (ret) {
			DBG("msm_fb_suspend: can't turn off display!\n");
			return ret;
		}
		mfd->op_enable = FALSE;
	}
...
*/
	...
	default:
		if (mfd->panel_power_on) {
			...
			DBG("%s, TASS LCD off start, DISABLE ESD IRQ.\n",__func__);
#endif
...
// 20100909 hongkuk.son for COOPER.rle ( booting logo )
	...
	if (!load_565rle_image_onfb( "COOPER_CTC.rle",0,0)) ; /* Flip buffer */
#else
	...
	if (!load_565rle_image_onfb( "TOTORO_CTC.rle",0,0)) ; /* Flip buffer */
#endif

#if defined(CONFIG_MACH_TASS)
#if defined(CONFIG_MACH_TASS_CTC)
	if (!load_565rle_image_onfb( "TASS_CTC.rle",0,0)) ; /* Flip buffer */
#else
