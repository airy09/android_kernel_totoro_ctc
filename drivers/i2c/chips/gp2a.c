...
#if defined(CONFIG_MACH_TOTORO_CTC) && (CONFIG_BOARD_REVISION >= 0x02) //twkim add proximity sensor driver to totoro_ctc
static void gp2a_gpio_set(int set)
{
    int err = 0;
    int config_scl, config_sda, config_int, config_en;
    printk("gp2a_gpio_set set[%d]\n",set);
    config_scl = GPIO_CFG(SENSE_SCL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
    config_sda = GPIO_CFG(SENSE_SDA, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
    config_en = GPIO_CFG(SENSE_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
    //if(set == ENABLE)
        config_int = GPIO_CFG(GPIO_SENSE_OUT, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
    //else
        //config_int = GPIO_CFG(GPIO_SENSE_OUT, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
    
    err = gpio_tlmm_config(config_scl, GPIO_CFG_ENABLE);
    if (err) 
        printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, SENSE_SCL, err);
    err = gpio_tlmm_config(config_sda, GPIO_CFG_ENABLE);
    if (err) 
        printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, SENSE_SDA, err);
    err = gpio_tlmm_config(config_en, GPIO_CFG_ENABLE);
    if (err) 
        printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, SENSE_EN, err);
    err = gpio_tlmm_config(config_int, GPIO_CFG_ENABLE);
    if (err) 
        printk(KERN_ERR "%s: gpio_tlmm_config(%#x)=%d\n", __func__, GPIO_SENSE_OUT, err);

    gpio_set_value(SENSE_SCL, set);
    gpio_set_value(SENSE_SDA, set);
    gpio_set_value(SENSE_EN, set);
//    gpio_set_value(GPIO_SENSE_OUT, set);
...
static int gp2a_opt_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	{
#if defined(CONFIG_MACH_TOTORO_CTC) && (CONFIG_BOARD_REVISION >= 0x02) //twkim add proximity sensor driver to totoro_ctc
		config = GPIO_CFG(GPIO_SENSE_OUT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA);
#else
