		...
		unsigned long arg)
{
	...
	case ECS_IOCTL_GETDATA:
		...
		mutex_unlock(&gakm->lock);
              ...
              for(i=0;i<8;i++)
                printk("data[%d] = %d\n",i,rwbuf.data[i]);
              x = (short)((((unsigned short)(rwbuf.data[2]))<<8)+(unsigned short)(rwbuf.data[1]));
              y = (short)((((unsigned short)(rwbuf.data[4]))<<8)+(unsigned short)(rwbuf.data[3]));
              z = (short)((((unsigned short)(rwbuf.data[6]))<<8)+(unsigned short)(rwbuf.data[5]));
              printk("[ak8975] x = %d y = %d \t z = %d \n",x,y,z);
#endif              
...
static int akm8975_setup_irq(struct akm8975_data *akm)
{
	...
	int irq;
       printk("akm8975_setup_irq gpio : %d\n",pdata->gpio_data_ready_int);
#if 0
...
//       gpio_tlmm_config(GPIO_CFG(37, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE); 
