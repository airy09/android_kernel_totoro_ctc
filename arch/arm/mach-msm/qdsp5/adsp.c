		 ...
		 struct msm_adsp_ops *ops, void *driver_data)
{
	...
	*out = module;
	...
	DBG("module %s has been registered\n", module->name);
...
void msm_adsp_put(struct msm_adsp_module *module)
{
	...
	if (module->ops) {
		...
		DBG("module %s is already closed\n", module->name);
	}
	mutex_unlock(&module->lock);
}
		   ...
		   void *cmd_buf, size_t cmd_size)
{
	...
	while (((ctrl_word = readl(info->write_ctrl)) &
		...
		DBG("waiting for DSP write ready\n");
		udelay(2);
		cnt++;
	}
...
static void handle_adsp_rtos_mtoa_app(struct rpc_request_hdr *req)
{
	...
	DBG("rpc event=%d, proc_id=%d, module=%d, image=%d\n",
...
static irqreturn_t adsp_irq_handler(int irq, void *data)
{
	...
	if (cnt == 15)
		DBG("too many (%d) events for single irq!\n", cnt);
	return IRQ_HANDLED;
}
...
int msm_adsp_enable(struct msm_adsp_module *module)
{
	...
	case ADSP_STATE_ENABLING:
		DBG("module '%s' enable in progress\n", module->name);
		break;
	case ADSP_STATE_ENABLED:
		DBG("module '%s' already enabled\n", module->name);
		break;
	case ADSP_STATE_DISABLING:
		DBG("module '%s' disable in progress\n", module->name);
		rc = -EBUSY;
		break;
	}
	mutex_unlock(&module->lock);
