...
static unsigned int keypad_row_gpios_00[] = { 33, 34 }; /* GPIO_NUM for KBC(0) ~ KBC(7) */
static unsigned int keypad_col_gpios_00[] = { 42, 41, 40 };          /* GPIO_NUM for KBR(0) ~ KBR(5) */
...
#define KEYMAP_INDEX_00(row, col) ((row)*ARRAY_SIZE(keypad_col_gpios_00) + (col))

// Cooper H/W REV 03 Keypad
#define KEYMAP_INDEX_03(row, col) ((row)*ARRAY_SIZE(keypad_col_gpios_03) + (col))
...
static unsigned short keypad_keymap_surf_00[ARRAY_SIZE(keypad_row_gpios_00) *
	...
	[KEYMAP_INDEX_00(0, 1)] = KEY_VOLUMEUP,
	[KEYMAP_INDEX_00(0, 2)] = KEY_VOLUMEDOWN,
	[KEYMAP_INDEX_00(1, 0)] = KEY_HOME,
	[KEYMAP_INDEX_00(1, 2)] = KEY_END,
...
static struct gpio_event_matrix_info surf_keypad_matrix_info_03 = {
	...
	.output_gpios	= keypad_row_gpios_03,
	.input_gpios	= keypad_col_gpios_03,
	.noutputs	= ARRAY_SIZE(keypad_row_gpios_03),
	.ninputs	= ARRAY_SIZE(keypad_col_gpios_03),
	.settle_time.tv.nsec = 40 * NSEC_PER_USEC,
	.poll_time.tv.nsec = 20 * NSEC_PER_MSEC,
	.flags		= GPIOKPF_LEVEL_TRIGGERED_IRQ | GPIOKPF_DRIVE_INACTIVE |
			  GPIOKPF_PRINT_UNMAPPED_KEYS
