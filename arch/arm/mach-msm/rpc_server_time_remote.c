			   ...
			   struct rpc_request_hdr *req, unsigned len)
{
	...
	case RPC_TIME_TOD_SET_APPS_BASES: {
		...
		if ( alarm_is_suspend() ) {
			printk(KERN_ERR "--> ignored in alarm device suspend\n"); 
			return 0; 
		}
#ifdef DEBUG_CODE
		kt = alarm_get_elapsed_realtime(); 
		printk(KERN_INFO "---> 1) ELAPSED=%d\n", kt.tv.sec ); 
#endif
		getnstimeofday(&ts);
		rtc_hctosys();
		getnstimeofday(&tv);
#ifdef DEBUG_CODE
		printk(KERN_INFO "RPC OLD ts=%ld\n", ts.tv_sec); 
		printk(KERN_INFO "RPC NEW tv=%ld\n", tv.tv_sec); 
#endif		
		/* Update the alarm information with the new time info. */
		alarm_update_timedelta(ts, tv);
#ifdef DEBUG_CODE
		kt = alarm_get_elapsed_realtime(); 
		printk(KERN_INFO "---> 2) ELAPSED=%d\n", kt.tv.sec ); 
#endif
		return 0;
	}
