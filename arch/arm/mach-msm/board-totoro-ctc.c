...
/* Gopeace_iwlee ADC °ª( 12BIT_MAX )
 * EAR Connect ADC Table by I509.
 *  0   <= ADC <= 1000 ::  3POLE
 * 1050 <= ADC         ::  4POLE
 *________________________________________________________
 * 			   	|	 1.05K	|	  1.35K	    |		25k   |
 * Connect(ADC)	|	991-993 |	1178-1180	|	3071-3077 |
 *--------------------------------------------------------

 *
 * EAR Key ADC Table by I509.
 *___________________________________________________________________________________
 *				|    SEDN/END		|			UP			|		   DOWN			|
 * 				|  0Ohm	 |  104Ohm	|   139Ohm	|	270Ohm	|	330Ohm	|	680Ohm  |
 * 4pole(1.35K)	|	0	 |		125	|		146	|		280	|		298	|		473	|
 * 4pole(25k)	|	0	 |		135	|		164	|		301	|		362	|		636	|
 *----------------------------------------------------------------------------------
*/
...
#ifdef FEATURE_SEC_JACK_VOLUME_KEY
static int get_stealth_ear_key_state(void)
#else
static int get_stealth_send_key_state(void)
#endif
{
	
	//pr_err("sec_jack: get_stealth_ear_key_state %s \n", gpio_get_value(GPIO_SEND_END)? "GPIO_OK" : "GPIO_NO");
	/* Active High */
...
static void set_stealth_micbias_state(bool state)
{
	...
	pr_err("sec_jack: set_stealth_micbias_state %s \n",data1?"on":"off");
...
#ifdef FEATURE_SEC_JACK_VOLUME_KEY
	.get_ear_key_state	= get_stealth_ear_key_state,
#else
...
#if defined(CONFIG_FB_MSM_LCDC_ILI9341_QVGA)
static void lcdc_ili9341_config_gpios(int enable)
{
	config_lcdc_gpio_table(lcdc_gpio_table,
		ARRAY_SIZE(lcdc_gpio_table), enable);
}

static struct msm_panel_common_pdata lcdc_ili9341_panel_data = {
	.panel_config_gpio = lcdc_ili9341_config_gpios,
	.gpio_num          = gpio_array_num,
};

static struct platform_device lcdc_ili9341_panel_device = {
	.name   = "lcdc_ili9341_qvga",
	.id     = 0,
	.dev    = {
		.platform_data = &lcdc_ili9341_panel_data,
	}
};
#endif
...
static void __init bt_power_timing_adjust_for_HW(void)
{
...
#if defined(CONFIG_SENSORS_BMA_ACCEL)
...
#if defined  (CONFIG_SENSORS_MMC328X) //twkim add mmc328x magnetic driver
...
static struct akm8975_platform_data akm8975_pdata = {
		.gpio_data_ready_int = 37, //PM8058_GPIO_PM_TO_SYS(PM8058_GPIO(33)), 
...
#if defined(CONFIG_SENSORS_GP2A)
static struct i2c_gpio_platform_data prox_i2c_gpio_data = {
        .sda_pin    = PROX_SDA,
        .scl_pin    = PROX_SCL,
        .udelay  = 3,  //// brian :3
        .timeout = 100,
...
static struct platform_device prox_i2c_gpio_device = {
        .name       = "i2c-gpio",
        .id     = 0x5,
        .dev        = {
...
static struct platform_device *devices[] __initdata = {
	...
	&acc_i2c_gpio_device,	
#endif
#if defined(CONFIG_SENSORS_MMC31XX) || defined(CONFIG_SENSORS_MMC328X) || defined(CONFIG_SENSORS_AK8975C) //twkim add mmc328x magnetic driver
	&mag_i2c_gpio_device,
#endif
#if defined(CONFIG_SENSORS_TAOS) || defined(CONFIG_SENSORS_GP2A)
	&prox_i2c_gpio_device,
#endif
...
#endif
#ifdef CONFIG_FB_MSM_LCDC_ILI9341_QVGA
	&lcdc_ili9341_panel_device,
#endif
/***/
...
static void sdc_set_gpio_disable(void)
{
...
static void proxi_set_gpio_disable(void)
{
    int err = 0;
...
static void __init msm7x2x_init(void)
{
...
#if defined(CONFIG_SENSORS_BMA_ACCEL)
	i2c_register_board_info(4, acc_i2c_devices, ARRAY_SIZE(acc_i2c_devices));
#endif
#if defined(CONFIG_SENSORS_TAOS) || defined(CONFIG_SENSORS_GP2A)
	i2c_register_board_info(5, prox_i2c_devices, ARRAY_SIZE(prox_i2c_devices));
#endif
#if defined(CONFIG_SENSORS_MMC31XX) || defined(CONFIG_SENSORS_MMC328X) || defined(CONFIG_SENSORS_AK8975C) //twkim add mmc328x magnetic driver
	i2c_register_board_info(6, mag_i2c_devices, ARRAY_SIZE(mag_i2c_devices));
#endif
#if defined(CONFIG_MACH_TOTORO_CTC) && (CONFIG_BOARD_REVISION >= 0x02) //twkim add proximity sensor driver to totoro_ctc
       proxi_set_gpio_disable();
#endif
	...
	{
#if 0 //defined(CONFIG_MACH_TASS_CTC)
		if(board_hw_revision>5)
			platform_device_register(&keypad_device_surf_6);
		else
#endif
...
#if 0//defined(CONFIG_MACH_TASS_CTC)
	if(board_hw_revision>5)
		keypad_gpio_control(&keypad_device_surf_6);
	else
#endif
	...
	msm7x2x_init_mmc();
#ifdef CONFIG_MACH_TOTORO_CTC
	bt_power_timing_adjust_for_HW(); //Always turn off after turning on before init BT 
#endif	
	bt_power_init();
