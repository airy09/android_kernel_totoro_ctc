...
void msm_pm_set_max_sleep_time(int64_t max_sleep_time_ns)
{
	...
	DBG(MSM_PM_DEBUG_SUSPEND, KERN_INFO,
...
int msm_reboot_call_is = 0; 
EXPORT_SYMBOL(msm_reboot_call_is); 

static int msm_reboot_call
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	msm_reboot_call_is = 1; 
 	printk("msm_reboot_call++\n");	

	if ((code == SYS_RESTART) && _cmd) {
		char *cmd = _cmd;
		if (!strcmp(cmd, "bootloader")) {
 			printk("command bootloader\n");	
			restart_reason = 0x77665500;
		} else if (!strcmp(cmd, "recovery")) {
#if defined(CONFIG_MACH_EUROPA) || defined(CONFIG_MACH_CALLISTO) || defined(CONFIG_MACH_COOPER) || defined(CONFIG_MACH_BENI) || defined(CONFIG_MACH_TASS) || defined(CONFIG_MACH_LUCAS) 
			set_recovery_mode();
#endif
 			printk("command recovery\n");	
			...
			restart_reason = 0x77665503;
 			...
 			printk("command download\n");	
			restart_reason = 0x776655FF;
		} else if (!strcmp(cmd, "eraseflash")) {
 			printk("command eraseflash\n");	
