...
static int __init do_early_param(char *param, char *val)
{
	...
	if ( (strcmp(param, "hw") == 0 ) )
	{
		...
		// HW_REV00 val = 6, HW_REV01 val = 5, HW_REV02 val = 5 // $ adb shell dmesg | grep revision
		if (strcmp("always_use_rev6", "yes") == 0) // to use this if (strcmp(..., "no") != 0)
			board_hw_revision = 6; // GOPEACE leepg EF26 -- Fix tempoary hwrev to enter download mode by key
		else if (strcmp(val, "5") == 0)
			board_hw_revision = 6; // GOPEACE iskim EF04 -- 임시로 백본모델의 HW REV을 사용함 (초기작업 완료후 반드시 삭제할것 !!)
		else
#endif
		if (strcmp(val, "1") == 0)
			board_hw_revision = 1;
			...
			board_hw_revision = 0;
		printk("Original H/W revision : 0x0%.4s\n", val);
